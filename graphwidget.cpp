...
void graphwidget::paintEvent(QPaintEvent *event) {
    ...
    long double delimeter1 = 10000*4.9;//для 0.0001
    ...
    long double delimeter5 = 10*5.1*1.16567;//для 0.1 и уменьшения темпы

    if(std::abs(radius - 0.0001) < 0.0000001)
    {
        delimeter = delimeter1;
    }
    else if(std::abs(radius - 0.001) < 0.0000001)
    {
        delimeter = delimeter2;
    }
    else if(std::abs(radius - 0.01) < 0.0000001)
    {
        delimeter = delimeter3;
    }
    else
    {
        if(temperatureValues[temperatureValues.size()-1] - temperatureValues[0] > 0)
        {
            delimeter = delimeter4;
        }
        else
        {
            delimeter = delimeter5;
        }
    }
...
//    }
//
//    // Красная линия для температуры газа
//    painter.setPen(Qt::red);
//    int yTarget = height - 50 - static_cast<int>((targetTemperature / maxTemperature) * (height - 100));
//    painter.drawLine(50, yTarget, width - 50, yTarget);
//    painter.drawText(5, yTarget, QString::number(targetTemperature, 'f', 2));
//
//    // Отметка точки пересечения на оси X
//    painter.setPen(Qt::black);
//    int xIntersection = 50 + static_cast<int>((total_time / maxTime) * (width - 100));
//    painter.drawLine(xIntersection, height - 50, xIntersection, 50); // Вертикальная линия в точке пересечения
//    painter.drawText(xIntersection, height - 40, QString::number(total_time, 'f', 2) + " s");
//
//    // Разметка оси X равными промежутками от точки пересечения
//    int xStep = (width - 100) / 10; // Шаг для отметок, настроим на 10 равных промежутков
//    for (int i = -5; i <= 5; ++i) {
//        int xPos = xIntersection + i * xStep;
//        if (xPos < 50 || xPos > width - 50) continue; // Пропуск отметок за границами графика
//
//        long double labelTime = total_time + i * (maxTime / 10); // Вычисление времени для метки
//        painter.drawLine(xPos, height - 50, xPos, height - 45); // Небольшая черточка для отметки
//        painter.drawText(xPos - 10, height - 30, QString::number(labelTime, 'f', 2)); // Метка времени
//    }
//
//    // Общее время
//    std::string str = "Total Time (blue): " + std::to_string(total_time) + " s";
//    QString totalTimeText = QString(str.c_str());
//    painter.drawText(50, height - 10, totalTimeText);
//}
